...
func (p *SQLiteMemoryProvider) trimHistory(ctx context.Context) error {
	p.mu.RLock()
	maxHistoryMessages := p.maxHistoryMessages
	sessionID := p.sessionID
	tableName := p.tableName
...
func (p *SQLiteMemoryProvider) CompressMemory(llm types.LLMProvider, maxMessages int) error {
	...
	now := time.Now()
	compressedMessages := make([]SQLiteMessageDocument, 0, len(systemMessages)+1+len(recentMessages))

	for _, msg := range systemMessages {
		compressedMessages = append(compressedMessages, SQLiteMessageDocument{
			SessionID: sessionID,
			Role:      msg.Role,
			Content:   msg.Content,
			Name:      msg.Name,
	...
	var insertedDocs []SQLiteMessageDocument
	...
	if err := tx.Table(tableName).
		Where("session_id = ? AND created_at < ?", sessionID, now).
		Delete(&SQLiteMessageDocument{}).Error; err != nil {
